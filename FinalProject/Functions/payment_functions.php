...
class paymentFunctions {
	...
	function getPaymentOptionsForUser($id) {
		require('C:\xampp\htdocs\FinalProject\DB\db_connect.php');
		$array = [];
		$stmt = $db->query('SELECT * FROM payments WHERE user_id = ' . $id);
	...
	function modifyPayment($id, $userID, $cardNumber, $securityCode, $expDate) {
		require('C:\xampp\htdocs\FinalProject\DB\db_connect.php');
	...
	function deletePayment($id, $userID) {
		require('C:\xampp\htdocs\FinalProject\DB\db_connect.php');
		$stmt = $db->query('DELETE FROM payments WHERE ID = ' . $id);
		$stmt->fetch();
		echo "<script>window.location.href='../profile.php?id=".$userID."';
