...
<main class="container" >
    <div class="bg-light p-5 rounded" style="height: 875px;">
        <h1 style="text-align: center;">Smash Burger</h1>
                ...
                <?php for($i = 0; $i < count($meals); $i++):?>
                    <div style="width: 100% height: 150px; margin-left: .01px; border: 1px solid black;">
                        ...
                        <div style="margin-left: 160px">
                            ...
                            <a href="../Functions/manage_cart.php?name=<?=$meals[$i]['name']?>&price=<?=$meals[$i]['price']?>&typeID=<?=$_GET['typeID']?>&rID=<?=$_GET['rID']?>&function=add" style="height: 10px" >Add to Cart</a>
                        </div>
                    </div>
                <?php endfor;?>
            </div>
            ...
            <div style="border: 1px solid black; height: 600px; overflow-y: auto;">
                <?php for($i = 0; $i < count($cart); $i++):?>
                    <div style="margin-left: 5px;">
                        <a style="float: right; margin-right: 10px;" href="../Functions/manage_cart.php?index=<?=$i?>&typeID=<?=$_GET['typeID']?>&rID=<?=$_GET['rID']?>&function=remove">Remove from Cart</a>
                        <h3><?=$cart[$i][0]?></h3>
